...
| `mq.ssl.keystore.content`                       | Base64-encoded contents of the JKS keystore to use for SSL (TLS) connections. Introduced to eliminate the need for volume mounts and to enable seamless use of ConfigProviders for managing keystore secrets in cloud‑native environments. `mq.ssl.keystore.location` and `mq.ssl.keystore.content` are mutually exclusive. Only one should be configured at a time. Supports two approaches: (1) Inline base64 string directly in config, or (2) Reference to base64 content via ConfigProvider (e.g., `${directory:/path:file}`) | string  |                | Base64-encoded JKS keystore content                                                                                                                                                                 |
...
| `mq.ssl.truststore.content`                     | Base64-encoded contents of the JKS truststore to use for SSL (TLS) connections. Introduced to eliminate the need for volume mounts and to enable seamless use of ConfigProviders for managing keystore/truststore secrets in cloud‑native environments. `mq.ssl.truststore.location` and `mq.ssl.truststore.content` are mutually exclusive. Only one should be configured at a time. Supports two approaches: (1) Inline base64 string directly in config, or (2) Reference to base64 content via ConfigProvider (e.g., `${directory:/path:file}`) | string  |                | Base64-encoded JKS truststore content                                                                                                                                                               |
